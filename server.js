...
app.get('/health', (_req, res) => res.json({ status: 'ok', stored_messages: readMessages().length, ai_configured: Boolean(OPENAI_API_KEY) }));
app.get('/webhook', (req, res) => { if (req.query['hub.mode'] === 'subscribe' && req.query['hub.verify_token'] === VERIFY_TOKEN) return res.status(200).send(req.query['hub.challenge'] || ''); return res.sendStatus(403); });
app.post('/webhook', (req, res) => { if (!verifySignature(req)) return res.sendStatus(401); const count = ingest(req.body); console.log('WhatsApp event stored; total:', count); return res.sendStatus(200); });
